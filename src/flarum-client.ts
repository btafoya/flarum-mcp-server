...
export class FlarumClient {
  ...
  async createTag(params: CreateTagParams): Promise<Tag> {
    ...
    if (params.color !== undefined) attributes.color = params.color;
    if (params.icon !== undefined) attributes.icon = params.icon;
    if (params.isHidden !== undefined) attributes.isHidden = params.isHidden;
    if (params.isRestricted !== undefined) attributes.isRestricted = params.isRestricted;
  ...
  private parseTags(response: JsonApiResponse<TagAttributes>): Tag[] {
    ...
    const tags = data.map((item) => {
      ...
      if (parentRef && !Array.isArray(parentRef)) {
        ...
        if (parent) {
          const attrs = (parent as { attributes: TagAttributes }).attributes;
          tag.parent = {
            id: parentRef.id,
            name: attrs.name,
            slug: attrs.slug,
